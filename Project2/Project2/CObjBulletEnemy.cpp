...
void CObjBulletEnemy::Draw()
{
	...
	if (drug3 != nullptr)
	{
		...
		{
			//表示位置の設定
			dst.m_top = -10.0f + m_y + drug3->GetScroll2();
			dst.m_left = -5.0f + m_x + drug3->GetScroll();
			dst.m_right = 45.0f + m_x + drug3->GetScroll();
	...
	if (instboss != nullptr)
	{
		...
		{
			//表示位置の設定
			dst.m_top = -10.0f + m_y + instboss->GetScroll2();
			dst.m_left = -5.0f + m_x + instboss->GetScroll();
			dst.m_right = 45.0f + m_x + instboss->GetScroll();
			dst.m_bottom = 32.0f + m_y + instboss->GetScroll2();
